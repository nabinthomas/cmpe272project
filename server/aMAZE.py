...
"""
    API Usage:
        ...
        - GET  /api/loginsuccess API to pass successful user auth from auth0
        - GET  /api/listings/<string:listings_id> API to get a particular listing details
        - POST /api/listings API to get listings 
"""}
...
def get_listings():
    ...
    try :
        if page_index <= 0 : 
            ...
            start = (page_index -1) * list_per_index +1
        ...
        print ("Error in generating output :", sys.exc_info()[0])    
        response = {}
        returnCode = ReturnCodes.ERROR_INVALID_PARAM
    return encodeJsonResponse(response, returnCode);
...
def get_one_listing(listings_id):
    ...
    try :
        ...
        reviews = db.reviews.find({"listing_id":listings_id}).sort( "date",  -1 )
        resp_reviews =[]
